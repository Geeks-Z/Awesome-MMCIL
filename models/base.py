...
class BaseLearner(object):
    def __init__(self, args):
        ...
        eval_topk = args.get("eval_topk", [5])
        if isinstance(eval_topk, int):
            eval_topk = [eval_topk]
        eval_topk = sorted({1, 5, *[int(k) for k in eval_topk if int(k) > 0]})
        self.eval_topk = eval_topk
        self.topk = max(self.eval_topk)
    ...
    def _evaluate(self, y_pred, y_true):
        ...
        matches = y_pred.T == np.tile(y_true, (y_pred.shape[1], 1))
        for k in self.eval_topk:
            if k == 1:
                continue
            effective_k = min(k, y_pred.shape[1])
            ret["top{}".format(k)] = np.around(matches[:effective_k].sum() * 100 / len(y_true), decimals=2)
        return ret
    ...
    def _eval_zero_shot(self):  
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            predicts = self._select_topk(outputs)
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            predicts = self._select_topk(outputs)  # [bs, topk]
            y_pred.append(predicts.cpu().numpy())
            y_true.append(targets.cpu().numpy())
