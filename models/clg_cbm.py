...
#        self.rest_cls = self.data_manager.concept_order
    ...
    def cluster(self,concept_cls,num_attributes=None):
        ...
        for start in range(0, len(attributes), self.batch_size):
            # Prompting batch by batch
            sub_attributes = attributes[start: start + self.batch_size]
        ...
        drop_singleton_batch = len(train_score_dataset) > 1 and len(train_score_dataset) % self.batch_size == 1
        train_loader = DataLoader(train_score_dataset, batch_size=self.batch_size, drop_last=drop_singleton_batch, shuffle=True, num_workers=num_workers)
