...
The log-derived results are maintained in [MMCL实验结果.xlsx](logs/MMCL实验结果.xlsx). The workbook has one sheet for each seed, `Seed0`, `Seed42`, `Seed1993`, and `Seed2026`, plus a `Benchmark` sheet for reference comparisons. The tables below preserve the reproduction context and historical comparisons; when a value differs, the workbook and its corresponding log are the source of truth.
