...
class DataManager(object):
    ...
    def get_attributes(self,attribute,indice):
        ...
        if attribute == 'random':
            ...
            concepts_root = os.path.join("utils", "clg_cbm", "concepts")
            concept_files = {
                "aircraft": "aircraft.json",
                "cars": "cars_4o_simple_cpts.json",
                "cifar": "concepts.json",
                "cifar100": "concepts.json",
                "cifar224": "concepts.json",
                "cub": "cub200_4o_simple_cpts.json",
                "cub200": "cub200_4o_simple_cpts.json",
                "food": "food_4o_simple_cpts.json",
                "food101": "food_4o_simple_cpts.json",
                "imagenetr": "imagenetr.json",
                "inr": "imagenetr.json",
                "objectnet": "objectnet.json",
                "sun": "sun.json",
                "ucf": "ucf101.json",
                "ucf101": "ucf101.json",
            }
            concept_file = concept_files.get(attribute.lower())
            if concept_file is None:
                raise NotImplementedError
            path = os.path.join(concepts_root, concept_file)
            if not os.path.exists(path):
                raise FileNotFoundError(f"Concept file for {attribute} not found: {path}")
        attr,cpt_count = [],[0]
    ...
    def get_prefix(self,attribute):
        ...
        elif name in ('cifar', 'cifar100', 'cifar224'): return "A bad photo of an object with "
        elif name in ('cub', 'cub200'): return "The bird has "
        elif name == 'imagenet-r': return "A picture of an object with "
        elif name == 'imagenet-a': return "A photo of an object with "
        elif name in ('food', 'food101'): return "A photo of the food with"
        ...
        elif name in ('ucf', 'ucf101'): return "A photo of a person with "
        elif name in ('inr', 'imagenetr'): return "A photo of the object with "
