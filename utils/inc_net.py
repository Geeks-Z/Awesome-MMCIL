    ...
    def update_stat(self, known_classes, total_classes, train_loader, device):
        ...
        cov_inv = center_vecs_norm.shape[1] * torch.linalg.pinv(
            (center_vecs_norm.shape[0] - 1) * center_vecs_norm.T.cov() + center_vecs_norm.T.cov().trace() * torch.eye(
                center_vecs_norm.shape[1], device=center_vecs_norm.device))
        current_ps = torch.ones(mu_norm.shape[0], device=mu_norm.device) * 1. / mu_norm.shape[0]
        ...
        ps = torch.ones(self.mu_norm.shape[0], device=self.mu_norm.device) * 1. / self.mu_norm.shape[0]
