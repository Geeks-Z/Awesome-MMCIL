...
# Continuum 1.0 no longer exports OfficeHome/DomainNet.  They are not used by
# the MMCL CIFAR-100/ImageNet-R protocol, so avoid blocking those runs on an
# unrelated optional dataset import.
from continuum.datasets import CIFAR100, ImageNet1000, ImageFolderDataset, TinyImageNet200, Core50, CUB200
...
def gen_dataset(args):
    if args["dataset"] == 'IMAGENET1K':
        ...
        label_path = os.path.join(args["data_root"], 'ImagenetR.txt')
        if not os.path.exists(label_path):
            label_path = os.path.join(args["file_root"], 'ImagenetR.txt')
