...
num_shards="${#gpus[@]}"
queue_root="${ROOT}/logs/OpenCLIP_LAION400M_ViTB16/queue/RanPAC-FeCAM"
mkdir -p "${queue_root}"
...
for shard_id in "${!gpus[@]}"; do
  ...
  nohup setsid env GPU_ID="${gpu_id}" SHARD_ID="${shard_id}" NUM_SHARDS="${num_shards}" \
    bash "${ROOT}/scripts/run_ranpac_fecam_shard.sh" \
    > "${output}" 2>&1 < /dev/null &
  pid="$!"
  printf '%s\n' "${pid}" > "${pid_file}"
